...
def detect_auth_event(line):
	if "Failed password for root" in line or "Invalid password for root" in line:
		event_type="ROOT_ATTACK"
		severity="HIGH"
		ip=extract_ip(line)
		user=extract_user(line)
		record_event(event_type,severity,ip,user)
...
def detect_http_probe(line):
	patterns=["nikto", "sqlmap", "namp", "nuclei"]
	for pattern in patterns:
		if pattern.lower() in line.lower():
			return True
	return False

def detect_process_anomaly(line):
	if "pam_unix" in line.lower() and "authentication failure" in line.lower():
		return True
	elif "not in sudoers" in line.lower():
		return True
	elif "execve" in line.lower() and "/tmp/" in line.lower():
		return True
	return False

#Record the events and finding the highest sever event
def record_event(event_type,severity,ip,user=None):
...
def get_events():
	...
	if event_type:
		...
		return Response(json.dumps(filtered, indent=4),mimetype="application/json")
...
try:
	...
	while True:
		...
		access_line=access_file.readline()
		if not auth_line and not sys_line and not access_line:
			time.sleep(0.5)
			continue
		event_type = None
		severity = None
		...
		if event_type=="SSH_FAILURE":
			...
			if len(ip_failure[key])>=5:
				...
				print(f"Severity       : {severity}")
				print(f"IP             : {ip}")
				print(f"Description    : 5 failure in 60 seconds")
				print("------------------------------------------------------------\n")
		...
		if access_line and detect_sql_injection(access_line):
			event_type="SQL_INJECTION"
			severity="HIGH"
			ip=extract_ip(access_line)
			record_event(event_type,severity,ip)
			print(f"[{timestamp}] {event_type} | {severity} | IP : {ip}")
			print("------------------------------------------------------------")
			print(f"Timestamp      : {timestamp}")
		...
		if access_line and detect_http_probe(access_line):
		...
		if sys_line and detect_process_anomaly(sys_line):
		...
		if result:
			src_ip,dpt=result
			if dpt in ["135", "445", "3389"]:
				event_type="FIREWALL_BLOCK"
				severity="LOW"
				record_event(event_type,severity,src_ip)
				print(f"[{timestamp}] {event_type} | {severity} | IP : {src_ip} | DPT : {dpt}")
				print("------------------------------------------------------------")
				print(f"Timestamp      : {timestamp}")
				print(f"Event Type     : {event_type}")
				print(f"Severity       : {severity}")
				print(f"Source IP      : {src_ip}")
				print(f"Blocked Port   : {dpt}")
				print("------------------------------------------------------------\n")
		...
		if time.time()-last_report>=60:
			...
			print("-----------------------------")
			print(f"Event            : {highest_event}")
			print(f"Severity         : {highest_severity}")
			for ip in ip_data:

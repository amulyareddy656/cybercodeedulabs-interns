	...
	while True:
		...
		if sys_line:
			...
			if result:
				...
				print(f"[{timestamp}] {event_type} | Severity : {severity} | SRC_IP : {src_ip} | DPT : {dpt}")
				record_event(event_type,severity,src_ip)
				if len(port_scn_dict[src_ip])>=3:
					event_type="PORT_SCAN"
					severity="MEDIUM"
					print(f"[{timestamp}] {event_type} | Severity : {severity} | IP : {src_ip}")
					record_event(event_type,severity,src_ip)
